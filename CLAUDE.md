...
黑盒 HTTP 打 4 个后端（`msw`:5100 / `nextjs`:5101 / `aspnetcore`:5104 / `springboot`:5105），
...
- **msw 是 oracle**：打 `:5100` 必须绿。红了先怀疑套件写错，而不是后端错
...
- **100% 覆盖 shared SSOT**：本仓端点清单 = 同家族 `<family>-shared/tsp/routes/*.tsp` 全集（`family` 取自本仓名 `*-contract-test` 的前缀；同家族任意一份 contract-test 共用同一条 SSOT 链路），auto-derive；禁止手挑；SSOT 有但本仓无测试 = L5 红
- **改端点必先改 SSOT**：增/删/改任何端点必须先改同家族 `<family>-shared/tsp/routes/*.tsp` 并重生 openapi；禁止在本仓声明 SSOT 之外的端点
...
1. **改端点**：先改 shared `tsp/routes/*.tsp` → `npm run build` 重生 openapi → 在 `tests/` 写测试
